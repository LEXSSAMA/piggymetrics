...
//ResourceServerTokenServices,它知道如何对收到的令牌进行解码,这个类是用来自定义ResourceServerTokenServices
//UserInfoTokenServices是ResourceServerTokenServices的实现类，CustomUserInfoTokenServices是
// 仿造UserInfoTokenServices来写的，UserInfoTokenServices的主要作用根据传入的accessToken来调用认证服务器中的/user端点提取出用户信息（UserDetails）
//其实就是改了getPrincipal和extractAuthentication两个方法,在添加了一个getRequest()方法
...
public class CustomUserInfoTokenServices implements ResourceServerTokenServices {
	...
	public OAuth2Authentication loadAuthentication(String accessToken)
		...
		//向传入的url发出GET请求，将返回的Json格式的数据装入Map中
